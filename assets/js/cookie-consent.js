/*!
 * TeamGalaxy Cookie Consent (single-file)
 * Drop this file into your site and include it on pages.
 * Replace GA_MEASUREMENT_ID with your GA4 id (G-XXXXXXXXXX).
 */
(function () {
  'use strict';
  ...
  // --- CSS injection (improved stylings) ---
  const css = `
  :root {
    --tg-bg: rgba(7,10,20,0.88);
    --tg-card: rgba(255,255,255,0.03);
    --tg-text: #e9f0ff;
    --tg-muted: #b9c7e0;
    --tg-accent: linear-gradient(90deg,#6b4bff,#4bd6ff);
    --tg-accent-solid: #6b4bff;
    --tg-radius: 12px;
    --tg-shadow: 0 10px 40px rgba(3,6,23,0.6);
  }
  .tg-cookie-banner {
    position: fixed;
    left: 1rem;
  ...
  .tg-visible { transform: translateY(0) scale(1); opacity: 1; }
  ...
  .tg-primary {
    ...
    background-image: var(--tg-accent);
    color: white;
    box-shadow: 0 6px 18px rgba(75,59,255,0.22);
  }
  .tg-secondary {
  ...
  .tg-modal[aria-hidden="false"] { display: flex; }
  .tg-modal-inner {
    width: 100%;
    max-width: 720px;
  ...
  .tg-modal-actions { display:flex; gap:10px; margin-top:10px; }
  ...
  @media (max-width: 740px) {
    ...
    .tg-emoji { align-self: flex-start; }
  ...
  @media (prefers-color-scheme: light) {
    :root {
      --tg-bg: rgba(255,255,255,0.98);
      --tg-card: #ffffff;
      --tg-text: #0b1220;
      --tg-muted: #44556f;
      --tg-accent-solid: #4b3bff;
      --tg-shadow: 0 10px 30px rgba(23,30,47,0.06);
    }
    .tg-cookie-inner { background: var(--tg-bg); }
    .tg-modal-inner { background: white; }
  }

  /* reduced motion */
  @media (prefers-reduced-motion: reduce) {
    .tg-cookie-inner { transition: none; transform: none; }
  }
  `;
  ...
  const inner = createEl('div', { class: 'tg-cookie-inner' });
  ...
  modalActions.appendChild(modalSave);
  ...
  modalInner.appendChild(modalP);
  modalInner.appendChild(toggleRow);
  modalInner.appendChild(modalActions);
  modalInner.appendChild(modalUtil);
  ...
  function loadGoogleAnalytics() {
    if (!GA_MEASUREMENT_ID || GA_MEASUREMENT_ID.indexOf('G-XXXXXXXX') === 0) {
  ...
  function showBanner() {
    ...
    setTimeout(() => { try { btnAccept.focus(); } catch(e){} }, 250);
  }
  function hideBanner() {
  ...
  function wireEvents() {
    ...
    // Close: treat as defer - hide now but do not store a consent
    ...
    // keyboard shortcut for dev: ctrl/cmd + c -> open preferences (non-intrusive)
  ...
  function checkExpiryAndPossiblyClear() {
    ...
    if (daysSince(c.timestamp) > CONSENT_EXP_DAYS) {
      removeConsent();
    }
  }
  ...
  function init() {
    ...
    // On page ready decide whether to show banner or load GA
    const existing = readConsent();
    if (existing && typeof existing.analytics !== 'undefined') {
      if (existing.analytics === true) loadGoogleAnalytics();
      // don't show banner if explicit choice exists
    } else {
      // show banner after small delay
      setTimeout(showBanner, SHOW_DELAY_MS);
    }

    // Expose developer helpers
